...
## 网络层 WAF 状态机与目标熔断（v023.3）

**为什么不能只看 HTTP 信号**：zueb.edu.cn 实测的封禁**没有任何 HTTP 层特征**——远程 RST（WinError 10054）→ ConnectionRefused（10061）→ 静默丢包（TimeoutError），429/验证码页/Turnstile 全都没有出现。

1. **信号分类（app/wafsignal.py）**：HTTP 层（429 / Retry-After / 挑战页 / WAF 页）+ 网络层（RST / 连接拒绝 / 连续超时 / DNS 失败）+ 工具输出文本特征（`classify_text`，供 shell 工具与脚本归因）。
2. **状态机**：`NORMAL → CAUTION → COOLDOWN → BLOCKED →（人工）MANUAL_PROBE → RECOVERED → NORMAL`。
   - 时间窗判定（默认 120s，`AGENT_WAF_SIGNAL_WINDOW`）：窗内 2×RST → COOLDOWN；2×连接拒绝 / 3×超时 → BLOCKED；429 / 挑战页 → 直接 COOLDOWN；**单次**网络错误只到 CAUTION（避免把本机抖动误判成封禁）。
   - CAUTION 自动降速：预算减半、突发 1、并发 1（不等 COOLDOWN 才动手）。
   - 状态只升不降：已 BLOCKED 不会被后续信号降级；解除只走人工探测 + 确认。
   - **同 IP 聚合暂停**：进入 COOLDOWN/BLOCKED 时，把解析到同一 IP 的兄弟根域名一并暂停（实测封禁是 IP 级，`www`/`jwc`/`lib`/`rsc` 一起不可达）。
3. **持久化**：状态落 `traffic_states`，**服务重启不清除**——重启绕过封禁这条路被堵死。
4. **恢复两步式**：`resume-probe`（用户手动，单次只读 GET，绕过状态检查但仍过 scope/限速）→ 成功进 RECOVERED（**不会自动恢复**）→ `confirm-resume` 才回 NORMAL。探测失败直接回 BLOCKED，不连续重试（探测本身会续期封禁）。
5. **失败归因独立**：Agent 侧新增 `WAF_BLOCKED` / `WAF_CAUTION` / `NETWORK` 归因档——遇到封禁状态时明确要求「停止自动请求，不要换参数/编码/工具/子域名继续」，与普通 L3 失败彻底分开。
6. 测试：`test_waf.py`（40 项：分类器、状态机转移、持久化、同 IP 聚合、两步恢复、不误判普通 403 / DNS 失败）。

## 低流量测试策略（v023.4）

1. **最小充分证据差分**：`diff-run` 默认 `evidence_mode=minimal`——基准 1 + 变体 1 = **2 个请求**即可判定；只有出现 `suspect_idor` 高价值差异才延迟 2 秒做一次真实复验 + 匿名对照（旧流程固定 4+ 请求）。`full` 模式保留旧行为。
2. **请求指纹复用能力**：同指纹（方法+规范化 URL+敏感头哈希+body 哈希+身份+变体）在 TTL 内复用已有观察，多分支测同一接口只有一次真实流量。**默认关闭**（`AGENT_TRAFFIC_FP_TTL=0`）——开启会改变差分/流程的既有语义（连续相同请求第二次命中缓存，稳定性判定失真），按目标敏感度显式开启。复验请求用独立 `variant_id` + `no_cache=True` 强制真实发出。
3. **split_task 同目标背压**：目标处于防护状态、或剩余预算低于 `AGENT_SUBTASK_MIN_BUDGET`（默认 6）时**拒绝并发拆分**——并行是请求量的放大器，不拆就不会有 N 路叠加。
4. **预算感知提示**：每轮把「目标剩余请求额度 + 当前防护状态」注入模型上下文（与步数预算提示并列），额度低时切换为收敛口径。

## 流量安全面板与审计报告（v023.5）

1. **前端「🛡 流量安全」面板**：目标状态卡（状态/解析 IP/预算用量/近 10 分钟请求数/并发/排队/最后错误）、开关按钮（暂停、单次恢复探测、确认恢复、清空排队、清除状态、暂停全部）、最近流量事件列表、一键导出流量审计。
2. **审计报告**（`GET traffic/report`，对齐 v023 计划第 8 节）：测试时长、实际发出请求数、被拒数、平均/最短间隔、按通道分布、按目标分布、状态码分布、网络层错误（含 errno）、防护/暂停事件、目标最终状态表。
3. **报告集成**：渗透测试报告新增「测试流量摘要」章节（含防护事件与目标最终状态），便于向 SRC 平台说明测试行为。
4. 测试：`test_traffic_audit.py`（事件聚合、报告章节、clear-queue 语义、pause-all 支撑、峰值并发记录）。
