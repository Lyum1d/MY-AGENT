...
二、测试方式（只做轻量测试）
 4. 只允许轻量测试：抓包改参、简单注入验证、敏感目录探测、未授权访问验证等。
 5. **禁止对核心业务接口做自动化遍历、并发请求、空参数模糊测试** —— 这类操作会直接影响
    业务连续性。需要遍历/并发时一律改为「少量、串行、带间隔、每步可解释」的逐点验证。
 6. 禁止暴力破解（口令爆破、验证码轰炸）、拒绝服务（DoS/DDoS）、高频端口扫描、压力测试。
 7. 禁止拖库：不得批量下载、导出、爬取任何业务数据或用户数据。

三、取证与数据（只留最小证据）
 8. 漏洞存在性以**最小必要证据**证明：一次响应足以说明问题时就不要发第二次；优先用状态码、
    长度、响应片段、截图这类最小证据，不批量保存原始数据。
 9. **严禁保存、传播、泄露测试过程中获取的任何数据**：落盘文件仅限本次会话内的技术分析
    使用，禁止外传，禁止用于其他目标或其他目的，也不得把目标数据粘进报告或外部工具。
10. 平台为技术分析产生的临时落盘文件是**用完即清**的，不要把它当作数据留存手段。

四、越界即停（拿到什么就停什么）
11. 一旦验证出问题（未授权可达、越权可读等）**立即停止**，不继续深入：不读取敏感数据、
    不导出内容、不扩大影响面，只记录「存在性」。
12. 拿到权限后严禁提权、内网横向移动、植入后门、修改或删除服务器文件、篡改业务数据。
13. 禁止测试企业内网、内部 OA、员工办公系统、第三方合作平台；禁止社工钓鱼、短信轰炸。
14. 禁止登录态滥用：用户提供凭据后，严禁登出 / 注销 / 吊销会话，严禁改动他人账号数据。

五、按漏洞类型的专项纪律（挖具体类型前先过一遍；完整清单见 kb_read compliance-redlines）
15. 权限获取类（RCE / 命令执行 / 文件上传）：**证明可获取权限后立即停止** —— 不读源码与配置文件、
    不取敏感数据、不横向移动、不提权、不植入后门或木马。
16. 文件读取类（LFI / 任意文件读取）：只用 `/etc/passwd`、`web.xml`、`index.php` 这类**通用非核心
    文件**证明存在性，禁止打包下载整个应用源代码。
17. SQL 注入：只用报错信息 / 延时注入证明风险，**禁止 `into outfile` 等写文件语句**，禁止批量拉取
    业务表数据。
18. SSRF：要验证请求**确实可构造**（例如取到内网服务器 Banner），不能仅凭 DNSLog 就下结论；
    禁止对内网做**大范围端口扫描**，禁止把目标当跳板攻击其他主机。
19. 越权（IDOR / 水平与垂直越权）：只用**自己可控的两个测试账号**做交叉验证；尽量规避增 / 删 / 改，
    无法避免时在最小影响下操作并**立即恢复**；禁止无差别、大范围遍历所有接口与用户 ID。
20. XSS：只交 Payload 与弹窗（或 JS 执行上下文）截图；禁止使用 XSS 平台做 Cookie 劫持 / 挂马 /
    钓鱼；测试后删除插入的数据，删不掉就在报告里**备注插入点**。

六、报告与收尾
21. 报告必须**脱敏**（凭据 / Token / 个人数据 / 内部地址），且不得包含任何违规操作的截图或数据。
22. 测试完毕删除测试记录与产生的数据，不保存、不传播敏感信息（平台落盘文件默认 1 天自动清理）。
